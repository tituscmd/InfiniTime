...
Music::Music(Pinetime::Controllers::MusicService& music) : musicService(music) {
  ...
  lv_style_set_bg_color(&btn_style, LV_STATE_DEFAULT, Colors::bgAlt); // Changed this from aqua to the nice dark-mode ahh gray
  ...
  lv_obj_align(txtArtist, nullptr, LV_ALIGN_IN_LEFT_MID, 12, MIDDLE_OFFSET + 2 * FONT_HEIGHT + LINE_PAD); // swapped txtTrack with txtArtists cuz thats how you do it
  lv_label_set_align(txtArtist, LV_ALIGN_IN_LEFT_MID);
  lv_obj_set_width(txtArtist, LV_HOR_RES - 12);
  lv_label_set_text_static(txtArtist, "Artist Name");
  lv_obj_set_style_local_text_color(txtArtist, LV_LABEL_PART_MAIN, LV_STATE_DEFAULT, Colors::lightGray); // looks goooood


  txtTrack = lv_label_create(lv_scr_act(), nullptr);
  lv_label_set_long_mode(txtTrack, LV_LABEL_LONG_SROLL_CIRC);
  lv_obj_align(txtTrack, nullptr, LV_ALIGN_IN_LEFT_MID, 12, MIDDLE_OFFSET + 1 * FONT_HEIGHT); // swapped txtTrack with txtArtists cuz thats how you do it
